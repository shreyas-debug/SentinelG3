...
async def run_scan(request: Request, body: ScanRequest):
    ...
    async def _stream() -> AsyncGenerator[str, None]:
        ...
        # ── Stage 3: Handle Patch Distribution ────────────
        if is_remote and healed_count > 0:
            if want_pr:
                # Create PR with token
                yield _sse_event("log", {
                ...
                # No token provided - inform user about their options
                yield _sse_event("log", {
                    "message": "▶ Stage 3 — Patch Review (No GitHub Token)"
                })
                yield _sse_event("log", {
                    "message": f"  ℹ {healed_count} fix(es) generated but NOT pushed to GitHub"
                })
                yield _sse_event("log", {
                    "message": "  📋 Review fixes below and manually apply them to your repo"
                })
                yield _sse_event("log", {
                    "message": "  💡 Tip: Add a GitHub token to automatically create a Pull Request"
                })
                
                # Emit a special event with instructions for manual application
                yield _sse_event("no_pr_info", {
                    "message": "Fixes generated but not pushed to GitHub",
                    "reason": "no_token",
                    "instructions": [
                        "Review the generated fixes in the 'Healing History' section below",
                        "Use 'Generate Fix' and 'Apply Fix' buttons for each vulnerability",
                        "Copy the fixed code and manually apply to your repository",
                        "Or provide a GitHub token to automatically create a Pull Request"
                    ],
